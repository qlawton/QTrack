...
@author: Quin7
"""
def COMPUTE_CURV_VORT_NON_DIV_UPDATE(data_in, data_out, res, radius, nondiv = True, SAVE_IMAGE = False, SAVE_OUTPUT = True):
    # IMPORT STATEMENTS
    import numpy as np
    import datetime
    ...
    if nondiv == False:
        u_wnd = nc_file.variables['u']
        v_wnd = nc_file.variables['v']
    else:
        u_wnd = nc_file.variables['upsi']
        v_wnd = nc_file.variables['vpsi']
    ...
    start = tm.time()
    Parallel(n_jobs=1)(delayed(run_loop)(i, radius) for i in np.arange(len(time)))
    end = tm.time()
