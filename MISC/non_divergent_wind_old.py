...
"""
Compute non-divergent wind for the 700hPa level, to be utilized later on in my
AEW tracking scripts. NOTE: ADAPTED FROM THE WINDSPHARM EXAMPLE ONLINE. 
...
def non_divergent_wind(file_in):
    ...
    wind_data = Dataset(nc_wind_file, 'r')
    ...
    # AEW wave tracking
    uwnd = np.nan_to_num(wind_data.variables['u'][:,:,:], nan = 0.0)
    lons = wind_data.variables['longitude'][:]
    lats = wind_data.variables['latitude'][:]
    vwnd = np.nan_to_num(wind_data.variables['v'][:,:,:], nan = 0.0)
    tm = wind_data.variables['time'][:]
    tm_units = wind_data.variables['time'].units
    wind_data.close()
