...
out_attr = 'EXTENDED'
year_int = np.arange(2020,2022,1)
...
for year in year_list:
    ...
    save_hov = '../../DATA/AEW/PROCESSED/Hovmoller/AEW_raw_tracks_updated_'+year+'_POST_'+out_attr+'.png'
    ...
    for TC_num in range(len(TC_id_list)):
        ...
        if (current_date[0] > datetime.datetime(int(year), 10, 31)) or (current_date[0] < datetime.datetime(int(year), 6, 1)) or np.min(current_lat)>=lat_cut:
            continue
    ...
    curvdata = 'extended_curv_vort_helmholz_700_R'+str(rad_used)+'_'+year+'_'+system+'.nc'
    ...
    for row in range(np.shape(AEW_lon)[0]):
        ...
        if row in TC_linked_num: #If the wave number is included in the wave list for linked TCs
            ...
            #TC_genesis_time_nc = [np.NaN]
            #TC_connect_name_nc = [np.NaN]
    print(AEW_gen_time_list)
    if save_data == True:
        import pickle
        data_save_out = '../../DATA/AEW/PROCESSED/extended_AEW_tracks_post_loop_updated_'+year+'.pkl'
    ...
    if save_data_nc == True: ## Also want to save the raw output as NC file for other scripts to use
        outfile = '../../DATA/AEW/TRACKS/extended_AEW_data_postprocessed_with_genesis_time_'+out_attr+str(year)+'_'+str(system)+'.nc'
        ...
        #ncout.createDimension('TC_gen_axis', None) #Unknown how long this will be...
        ...
        ## create TC_gen_axis
        #TC_gen_axis = ncout.createVariable('TC_gen_axis', np.dtype('float64').char, ('TC_gen_axis'))
        ...
        #TC_gen_axis[:] = np.arange(np.array(TC_genesis_time_nc).size)[:]
    ...
    if run_animation == True:
        ...
        #att = info_name
        dir_ani_frame = 'frames_'+system+'_'+year+'_R'+str(rad_used)+'edit_extended/'
        GIF_dir = save_dir
        file_list = []
        time_list = []
        lon_list = []
    ...
    if save_hovmoller == True:
        ...
        for TC_num in range(len(TC_id_list)):
            ...
            if (current_date[0] < datetime.datetime(int(year), 10, 31)) and (current_date[0] > datetime.datetime(int(year), 6, 1)) and np.min(current_lat)<=lat_cut:
            ...
            try:
                ...
                print('fail')
                pass
        ax.set_ylim([datetime.date(int(year), 6, 1), datetime.date(int(year), 10, 31)])
        plt.gca().invert_yaxis()
        ax.set_title("AEW Tracks: Jun1-Oct31 "+str(year), fontsize = 20)
